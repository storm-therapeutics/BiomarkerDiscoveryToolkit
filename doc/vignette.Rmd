...
Compound X, a novel enzyme-targeting inhibitor, is being investigated as an anti-cancer drug.
In a dose response study, X was tested for anti-proliferative effects in a diverse panel of cancer cell lines.
...
exp.mat <- filter.threshold(common$data, fraction=0.5) # require TPM >= 1 in half the samples
...
Although both the correlation-based and the group-based analysis aim to find assocations between drug sensitivity and gene expression across cell lines, they prioritise largely different genes:
...
```{r gsea_scores}
scores.cor <- -cors[, "cor.spearman"]; names(scores.cor) <- rownames(cors) # scores for cor.-based analysis
scores.test <- -log10(stats$p.value) * sign(stats$mean.responder - stats$"mean.non-responder")
names(scores.test) <- rownames(stats) # scores for group-based analysis
```
